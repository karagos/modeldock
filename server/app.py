...
ALLOWED_ORIGINS = {"http://127.0.0.1:8420", "http://localhost:8420"}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _request_allowed(self, write=False):
        """Blocks DNS-rebinding (Host check) and cross-site form POSTs
        (Origin + Content-Type). Non-browser callers (frontend fetch, MCP,
        curl) send no Origin or our own, and JSON content."""
        if self.headers.get("Host", "") not in ALLOWED_HOSTS:
            self._json({"error": "Rejected: unexpected Host header."}, 403)
            return False
        if write:
            origin = self.headers.get("Origin", "")
            if origin and origin not in ALLOWED_ORIGINS:
                self._json({"error": "Rejected: cross-origin request."}, 403)
                return False
            ctype = self.headers.get("Content-Type", "")
            if not ctype.startswith("application/json"):
                self._json({"error": "Rejected: JSON body required."}, 403)
                return False
        return True
    ...
    def do_GET(self):
        ...
        try:
            if route == "/api/ping":
                ...
                s = dict(STORE.data["settings"])
                s["has_hf_token"] = bool(s.get("hf_token"))
                s["hf_token"] = ""     # the secret never leaves the server
                self._json(s)
    ...
    def do_POST(self):
        route = self.path.split("?")[0]
        if not self._request_allowed(write=True):
            return
        try:
            body = self._body()
        ...
        if q.get("q", "").strip():
            ...
            for h in hf_api.search_fulltext(q.get("q", "").strip()):
                ...
                if want_bucket and catalog.size_bucket(p["total_b"] if p else None) != want_bucket:
                    continue
                if cutoff:
                    continue   # full-text hits carry no release date; a period window excludes them
                ft_cards.append({
    ...
    def _api_download(self, body):
        ...
        if not files:
            self._json({"error": "No files given for this download."}, 400)
            return
        total = sum(f["size"] for f in files)
        ...
        # Capabilities belong to the model, not to its folder name. Ask the Hub
        # once here so the manifest can carry the answer for good.
        try:
            tags = hf_api.model_info(mid).get("tags", []) or []
        except Exception:            # metadata must never block a download
            tags = []
        job = {"id": make_job_id(), "model_id": mid, "tags": tags,
