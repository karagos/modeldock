...
class DownloadManager:
    ...
    def pause(self, job_id):
        with self._lock:
            ...
            if job and job["state"] == "queued":
                # Not yet picked up by the worker: pause it directly, or the
                # signal would be dropped when the worker dequeues it.
                job["state"] = "paused"
                self.store.save()
                return
    ...
    def _download_file(self, job, f, final, done_bytes):
        ...
        attempts = 0
        while True:
            self._check_signal(job)   # honor pause/cancel even while offline
            if pos >= f["size"]:
            ...
            try:
                ...
                self._interruptible_sleep(job, min(2 ** min(attempts, 6), NET_BACKOFF_MAX))
    ...
    def _interruptible_sleep(self, job, seconds):
        """Backoff that still honors pause/cancel, checked every second."""
        slept = 0.0
        while slept < seconds:
            self._check_signal(job)
